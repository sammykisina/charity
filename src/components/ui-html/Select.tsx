  ...
  return (
    ...
    >
      <div className={`flex items-center justify-between gap-1`}>
        <div className="flex flex-1 gap-1 overflow-x-auto whitespace-nowrap text-sm font-semibold capitalize text-dark/90 scrollbar-hide">
          {selected.length === 0 || !selected
            ...
            ? selected.map((selected_option, selected_option_index) => (
                <div
                  ...
                  className="flex items-center rounded-full bg-yellow/20 px-2 tracking-wider"
                >
                  <Icon icon={<HiX />} />
                  <span>
        ...
        <span
          className={`ml-4 flex items-center justify-center rounded-full p-1 duration-300 hover:bg-dark hover:text-white ${
            isSelectPanelOpen && "rotate-180 rounded-full  bg-dark text-white"
          }`}
        >
      ...
      <ul
        className={`text-c_gray absolute left-0 top-[calc(100%+.25rem)] z-50 flex  h-fit  w-full
         flex-col gap-2 overflow-y-auto rounded-md  p-[5px] text-sm
